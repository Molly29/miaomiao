        ...
        <div v-else class="city_list">
            <div class="city_sort" ref="city_sort">
                <div v-for="item in cities" :key="item.index">
                    <h2>{{item.initial}}</h2>
                    <ul>
                        <li v-for="itemList in item.list" :key="itemList.index" @click = "handleToCity(itemList.name,itemList.code)">{{itemList.name}}</li>
                    </ul>
                </div>	
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        handleToCity(name,code){
            this.$store.commit('city/CITY_INFO',{name,code});
            window.localStorage.setItem('nowNm',name);
            window.localStorage.setItem('nowId',code);
